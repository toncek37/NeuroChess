...
def _sign(value: float, deadband: float = 25.0) -> int:
    if value > deadband:
        return 1
    if value < -deadband:
        return -1
...
def _recommend(metrics: ValueMetrics, orientation_passed: bool) -> str:
    if not orientation_passed:
        return "Orientation sanity test failed: verify side-to-move semantics before using neural value in search."
    if metrics.pearson_cp < 0.0:
        return "Model value is anti-correlated with the teacher. Check WDL target ordering/orientation or retrain."
    if metrics.pearson_cp < 0.30 or metrics.sign_agreement_percent < 60.0:
        return "Value head is too weak for a large search blend. Retrain on more/better-labelled positions; keep neural value disabled for Elo tests."
    if metrics.mae_cp > 250 or metrics.saturated_percent > 20.0:
        return "Value direction is useful but calibration is poor. Use a small blend (5–10%) and improve calibration/training before 50%."
    return "Value head looks directionally usable. Benchmark conservative blends first: 5%, 10%, then 20%."


SANITY_POSITIONS = (
    ("White queen advantage, White to move", "7k/8/8/8/8/8/4Q3/4K3 w - - 0 1", 1),
    ("White queen advantage, Black to move", "7k/8/8/8/8/8/4Q3/4K3 b - - 0 1", -1),
    ("Black queen advantage, White to move", "4k3/4q3/8/8/8/8/8/7K w - - 0 1", -1),
    ("Black queen advantage, Black to move", "4k3/4q3/8/8/8/8/8/7K b - - 0 1", 1),
    ("Bare kings, White to move", "7k/8/8/8/8/8/8/K7 w - - 0 1", 0),
    ("Bare kings, Black to move", "7k/8/8/8/8/8/8/K7 b - - 0 1", 0),
)
...
def run_value_debug(
    ...
    *,
    ...
    emit("Running side-to-move/material sanity positions...")
    sanity: list[SanityRow] = []
    for name, fen, expected in SANITY_POSITIONS:
        ...
        row = SanityRow(name, fen, expected, float(probs[0]), float(probs[1]), float(probs[2]), cp, passed)
        sanity.append(row)
        emit(f"  {name}: W/D/L={row.win:.3f}/{row.draw:.3f}/{row.loss:.3f}, cp={cp:+d} -> {'PASS' if passed else 'FAIL'}")

    orientation_passed = all(r.passed for r in sanity[:4])
    ...
    tc = np.asarray(teacher_cp, dtype=np.float64)
    ...
    decisive = np.abs(tc) > 25.0
    if np.any(decisive):
        sign_agreement = float(np.mean(np.sign(tc[decisive]) == np.sign(mc[decisive])) * 100.0)
    else:
        sign_agreement = 0.0
    ...
    metrics = ValueMetrics(
        ...
        mae_cp=float(np.mean(np.abs(tc - mc))),
        sign_agreement_percent=sign_agreement,
        teacher_mean_cp=float(np.mean(tc)),
    ...
    emit(f"Pearson teacher/model cp: {metrics.pearson_cp:.3f}")
    emit(f"MAE: {metrics.mae_cp:.1f} cp")
    emit(f"Sign agreement: {metrics.sign_agreement_percent:.1f}%")
