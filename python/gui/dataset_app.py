...
class DatasetApp(tk.Tk):
    ...
    def _build(self):
        ...
        ttk.Label(outer,text="Side-balanced Stockfish self-play dataset",font=("Segoe UI",16,"bold")).pack(anchor="w")
        ttk.Label(outer,text="Recommended Value Dataset v3: Stockfish-guided games balanced across all six combinations of side-to-move (White/Black) and teacher value (losing/equal/winning). This prevents the network from learning a color/turn shortcut.",wraplength=800).pack(anchor="w",pady=(4,16))
        ...
        ttk.Label(opts,text="Each of the six side/value groups receives approximately the same number of positions. Top moves adds controlled variety by sampling among Stockfish's strongest candidates.",wraplength=760).grid(row=2,column=0,columnspan=5,sticky="w",pady=(10,0))
        buttons=ttk.Frame(outer); buttons.pack(fill="x"); self.start_btn=ttk.Button(buttons,text="Generate side-balanced dataset",command=self.start); self.start_btn.pack(side="left"); self.stop_btn=ttk.Button(buttons,text="Stop",command=self.stop,state="disabled"); self.stop_btn.pack(side="left",padx=8); ttk.Label(buttons,textvariable=self.status).pack(side="right")
    ...
    def _pick_output(self):
        p=filedialog.asksaveasfilename(title="Save side-balanced labelled dataset",defaultextension=".jsonl",filetypes=[("JSON Lines","*.jsonl")],initialfile=f"training_side_balanced_{self.positions.get()}.jsonl");
    ...
    def start(self):
        ...
        self.start_btn.configure(state="disabled"); self.stop_btn.configure(state="normal"); self.progress.start(10); self.status.set("Generating side-balanced Stockfish self-play…")
        self._append(f"Target: {count} side-balanced labelled positions"); self._append(f"Stockfish: {engine}"); self._append(f"Teacher: depth {depth}, MultiPV {multipv}, sampled from top {top} moves"); self._append("Target groups: White/Black to move × losing/equal/winning, approximately equal size (threshold ±150 cp)")
    ...
    def _poll(self):
        try:
            while True:
                ...
                elif kind=="done": self._finish(); self.status.set("Dataset ready"); self._append(f"DONE: {value}"); messagebox.showinfo("Dataset ready",f"Side-balanced dataset created:\n{value}\n\nTrain a fresh model, export it to ONNX, then run Value Debugger before Elo testing.")
