...
'''
if old not in s:
    raise SystemExit('search reset insertion point not found')
s = s.replace(old, new, 1)

marker = '// Selective neural root guidance: at most 12 ONNX calls, once per move.'
start = s.index(marker)
end = s.index('    const int alpha_original = alpha;', start)
replacement = '''    // Neural root guidance is computed once per search, then reused at every
